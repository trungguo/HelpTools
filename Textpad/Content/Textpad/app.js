...
function getListNoted() {
    try {
        var listNoted = JSON.parse(localStorage.getItem("ListNoted") || "[]");
        return listNoted;
    } catch (e) {
        return null;
    }
...
function renderListNotedView() {
    try {
        var listNoted = getListNoted()
        listNoted.sort(function (a, b) {
        ...
        if (listNoted) {
            $("#ListNotedView ul").empty();
            listNoted.forEach(function (item, index, arr) {
                var li = `<li class="settings-menu__item" style="padding-top: 0.5rem; padding-bottom: 0.5rem"><a style="color: #12c2e9; text-decoration: underline" href="/${item.Id}">${item.FileName}</a></li>`
                $("#ListNotedView ul").append(li)
            });
        }
    } catch (e) {
        return
...
function setListNoted(listNoted) {
    try {
        localStorage.setItem("ListNoted", JSON.stringify(listNoted));
...
function clearListNoted() {
    try {
        localStorage.setItem("ListNoted", JSON.stringify([]));
        return;
    } catch (e) {
        return;
    }
...
function getNoted(id) {
    ...
    return listNoted.find(e => e.Id == id);
}

function addNotedName(newNoted) {
    var listNoted = getListNoted();
    if (!listNoted) {
        listNoted = []
    }
    listNoted.push(newNoted);

    setListNoted(listNoted);
}

function removeNoted(id) {
    var listNoted = getListNoted();
    if (!listNoted) {
        listNoted = []
    }

    var index = listNoted.findIndex(x => x.Id == id);
    if (index > -1) { // only splice array when item is found
        listNoted.splice(index, 1); // 2nd parameter means remove one item only
    }

    setListNoted(listNoted);
}

function setNotedName(id, name) {
    var listNoted = getListNoted();
    if (!listNoted) {
        return
    }

    var noted = listNoted.find(e => e.Id == id);
    if (!noted) {
        return
    }
    noted.FileName = name;
...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    var
        ...
        dowload = document.getElementById("dowload"),
    ...
    if (hasStorage() === true) {  //2
        ...
        title.addEventListener("keyup", function () {
            ...
            setNotedName(fileId.value, this.value)

        });

        //if (getNoted(fileId.value)) {
        //    var fileName = getNoted(fileId.value).FileName;
        //    if (fileName) {
        //        title.value = fileName.replace(".txt", "");
        //    }
        //}
    ...
    dowload.addEventListener("click", function (event) {
        event.preventDefault();
        var blob = new Blob([content.value || content.placeholder], { type: "text/plain;charset=utf-8" });

        if (title.value == "") {

            smoke.alert("Chưa đặt tên cho file", function (e) {
            }, {
                reverseButtons: true,
            });
    ...
    remove.addEventListener("click", function (e) {
        if (!fileId.value) {
            ...
            smoke.confirm("Bạn chắc chắn muốn xóa không?", function (e) {
                if (e) {
                    deleteFile()
                }
            }, {
    ...
    function saveFile(fileName) {
        var fd = new FormData();
        fd.append('Id', fileId.value);
        fd.append('FileName', fileName);
        fd.append('FileContent', content.value);
        $.ajax({
            type: 'POST',
            url: 'Note/Save',
            data: fd,
            ...
            success: function (result) {
                if (!fileId.value) {
                    addNotedName({ Id: result.data, FileName: fileName })
                } else {
                    setNotedName(result.data, fileName);
                }
    ...
    function downloadFile() {
        $.ajax({
            type: 'GET',
            url: 'Note/Download?id=' + fileId.value,
            processData: false,
            contentType: false,
            success: function (result) {
            },
    ...
    function deleteFile() {
        $.ajax({
            type: 'GET',
            url: 'Note/Delete?id=' + fileId.value,
            processData: false,
            contentType: false,
            success: function (result) {
                removeNoted(fileId.value)
                location.href = "/";
            },
            error: function (response, status, xhr) {
            },
